/* baby.stephens.page — pregnancy + baby tracker frontend.
   Vanilla JS single-page app. Offline-friendly: reads from a localStorage cache
   and queues new log entries when the network is down, flushing on reconnect. */
"use strict";
...
const state = {
  ...
  view: "home",
  filter: "all",
  settings: readJSON(LS.settings, {}),
  events: readJSON(LS.events, []),
  queue: readJSON(LS.queue, []),
...
let tmpSeq = -1;
function logEvent(type, data, opts = {}) {
  const ev = {
    id: "tmp" + (tmpSeq--),
    type, data,
  ...
  state.queue.push({ tmpId: ev.id, payload: { type, data, ts: ev.ts, note: ev.note, logged_by: ev.logged_by } });
  save();
  flush();
  return ev;
}
...
async function flush() {
  ...
  while (state.queue.length) {
    ...
    try {
      const real = await api("POST", "/events", job.payload);
...
async function patchEvent(id, patch) {
  ...
  const real = await api("PATCH", "/events/" + id, patch);
...
async function syncAll() {
  ...
  try {
    ...
    // keep any still-pending local events not yet on server
...
function units() {
  ...
  return {
    imperial,
    vol: { u: imperial ? "oz" : "ml", toC: (v) => imperial ? v * 29.5735 : v, fromC: (ml) => imperial ? ml / 29.5735 : ml },
    mass: { u: imperial ? "lb" : "kg", toC: (v) => imperial ? v / 2.20462 : v, fromC: (kg) => imperial ? kg * 2.20462 : kg },
    len: { u: imperial ? "in" : "cm", toC: (v) => imperial ? v * 2.54 : v, fromC: (cm) => imperial ? cm / 2.54 : cm },
  };
}
...
function phase() {
  const b = state.settings.birth_date;
  if (b && new Date(b) <= new Date()) return "baby";
  if (state.settings.due_date) return "pregnancy";
  return "setup";
}
function pregInfo() {
  const due = new Date(state.settings.due_date + "T00:00:00");
  const lmp = new Date(due.getTime() - 280 * DAY);
  const days = Math.floor((Date.now() - lmp.getTime()) / DAY);
  const weeks = Math.max(0, Math.floor(days / 7));
  const remDays = ((days % 7) + 7) % 7;
  const daysToDue = Math.ceil((due.getTime() - Date.now()) / DAY);
  const tri = weeks < 14 ? "First trimester" : weeks < 28 ? "Second trimester" : "Third trimester";
  const pct = Math.min(1, Math.max(0, days / 280));
  return { due, weeks, remDays, daysToDue, tri, pct };
}
function babyAge() {
  const b = new Date(state.settings.birth_date + "T00:00:00");
  const days = Math.floor((Date.now() - b.getTime()) / DAY);
  const w = Math.floor(days / 7);
  const months = Math.floor(days / 30.44);
  return { days, weeks: w, remDays: days % 7, months };
}
...
function summary(e) {
  ...
  switch (e.type) {
    ...
    case "diaper": return { wet: "Wet", dirty: "Dirty", both: "Wet + Dirty" }[d.kind] || "Diaper";
    case "sleep":
    ...
    default: return META[e.type]?.label || e.type;
  }
}
...
function phaseTag() {
  const p = phase();
  if (p === "pregnancy") { const g = pregInfo(); return `${g.weeks}w ${g.remDays}d`; }
...
function homeSetup() {
  return `<div class="stack">
    <section class="card reveal hero">
      ...
      <p class="due">Tell us your due date, or your baby's birthday, and we'll tailor everything from here.</p>
...
function homePreg() {
  ...
  const w = window.WEEK_DATA[wk] || window.WEEK_DATA[40];
  ...
  return `<div class="stack">
    <section class="card reveal hero">
      ...
      <div class="ring-wrap">
        ...
        <div class="ring-label"><span class="big">${g.weeks}<span style="font-size:1.3rem">w</span></span><span class="sub">${g.remDays} days</span></div>
      </div>
      <span class="trimester">${g.daysToDue >= 0 ? g.daysToDue + " days to go" : Math.abs(g.daysToDue) + " days over"}</span>
      <h2>Due ${g.due.toLocaleDateString([], { month: "long", day: "numeric", year: "numeric" })}</h2>
    </section>

    <section class="card reveal size-card">
      <div class="fruit">${w.emoji}</div>
      <div class="meta">
        <div class="eyebrow">Week ${wk} · size of a</div>
...
function homeBaby() {
  const a = babyAge();
  const last = (t) => state.events.find((e) => e.type === t);
...
function timelineView() {
  const types = phase() === "baby"
    ? ["all", "feed", "diaper", "sleep", "pump", "measurement", "milestone", "appointment", "note"]
    : ["all", "kick", "contraction", "weight", "symptom", "appointment", "note"];
  ...
  return `<div class="chips">${types.map((t) => `<button class="chip ${state.filter === t ? "on" : ""}" data-action="filter:${t}">${t === "all" ? "Everything" : META[t]?.label || t}</button>`).join("")}</div>
    ${keys.length ? keys.map((k) => `<div class="tl-day-label">${dayLabel(Number(k))}</div>${groups[k].map(tlItem).join("")}`).join("") : emptyState("📖", "No entries here yet")}`;
}
...
function toolCard(type, title, desc, action) {
  const m = META[type];
  ...
  <div class="stack">
    <section class="card">
      <form data-form="settings" class="stack" style="margin:0">
        ...
        <label class="fld"><span class="lt">Due date</span><input type="date" name="due_date" value="${esc(s.due_date || "")}"></label>
    ...
    <p class="tiny" style="text-align:center;padding:0 20px">Your passphrase is set on the server. To change it, update <code>PASSWORD_HASH</code> in the service env and restart. Everything here is private to your household.</p>
  </div>`;
}

/* ---------------------------------------------------- deferred drawing ---- */
function drawDeferred() {
  // animate the pregnancy ring after layout
  const fg = $(".ring-fg");
  if (fg) { const v = fg.getAttribute("stroke-dashoffset"); fg.setAttribute("stroke-dashoffset", fg.getAttribute("stroke-dasharray")); requestAnimationFrame(() => requestAnimationFrame(() => fg.setAttribute("stroke-dashoffset", v))); }
}

/* ------------------------------------------------------------ chrome ------ */
function nav() {
  const items = [["home", "Home", I.home], ["timeline", "Timeline", I.list], ["tools", "Tools", I.tools], ["settings", "Settings", I.gear]];
...
function closeSheet() { const s = $("#scrim"); if (s) { s.classList.remove("open"); setTimeout(() => s.remove(), 300); } stopTickers(); }

function nowLocalInput() { const d = new Date(); d.setMinutes(d.getMinutes() - d.getTimezoneOffset()); return d.toISOString().slice(0, 16); }
function tsField(label = "When") { return `<label class="fld"><span class="lt">${label}</span><input type="datetime-local" name="ts" value="${nowLocalInput()}"></label>`; }
function noteField() { return `<label class="fld"><span class="lt">Note <span class="tiny">(optional)</span></span><textarea name="note" placeholder="Anything to remember"></textarea></label>`; }

function logChooser() {
  const p = phase();
  const opts = p === "baby"
    ? ["feed", "diaper", "sleep", "pump", "measurement", "milestone", "appointment", "note"]
    : ["kick", "contraction", "weight", "symptom", "appointment", "milestone", "note"];
...
function submitRow(label = "Save") { return `<div class="btn-row"><button class="btn ghost" type="button" data-action="sheet-close">Cancel</button><button class="btn primary" type="submit">${label}</button></div>`; }

/* read a form's segmented + field values */
function formData(form) {
  const fd = {};
  form.querySelectorAll("input,select,textarea").forEach((el) => { if (el.name) fd[el.name] = el.value; });
  return fd;
}

function submitLog(form) {
  const type = form.dataset.type, f = formData(form), U = units();
  const ts = f.ts ? new Date(f.ts) : new Date();
  let data = {}, note = f.note || null, when = ts;
  if (type === "feed") {
...
/* ------------------------------------------------------- tools: kicks ---- */
let ticker = null, kickState = null, contrState = null;
function stopTickers() { if (ticker) { clearInterval(ticker); ticker = null; } kickState = null; contrState = null; }
...
function openKick() {
  ...
  ticker = setInterval(() => { const el = $("#kick-t"); if (el && kickState) el.textContent = fmtDur((Date.now() - kickState.start) / 1000); }, 1000);
}
function kickTap() { if (!kickState) return; kickState.count++; const el = $("#kick-n"); if (el) el.textContent = kickState.count; if (navigator.vibrate) navigator.vibrate(15); }
function kickSave() {
...
function openContraction() {
  ...
  renderContraction();
  ticker = setInterval(() => { if (contrState?.running) { const el = $("#contr-t"); if (el) el.textContent = fmtDur((Date.now() - contrState.start) / 1000); } }, 200);
}
function recentContractions() {
  return state.events.filter((e) => e.type === "contraction").slice(0, 8)
    .map((e) => ({ ts: new Date(e.ts).getTime(), dur: e.data.duration_sec, gap: e.data.since_last_sec }));
}
function renderContraction() {
  const l = contrState.list;
  const avgDur = l.length ? l.reduce((s, x) => s + x.dur, 0) / l.length : 0;
  const gaps = l.map((x) => x.gap).filter(Boolean);
  ...
  openSheet(`<h3>Contraction timer</h3><p class="sub">Hold tight. Press start when one begins, stop when it ends.</p>
    <div class="timer-face"><div class="tlab">${contrState.running ? "contraction" : "ready"}</div><div class="tval ${contrState.running ? "pulse" : ""}" id="contr-t">${contrState.running ? "0s" : "0s"}</div></div>
...
function contractionToggle() {
  ...
  if (!contrState.running) { contrState.running = true; contrState.start = Date.now(); renderContraction(); }
    ...
    contrState.list.unshift({ ts: contrState.start, dur, gap });
    renderContraction();
...
function openGrowth() {
  ...
  const pts = state.events.filter((e) => isBaby ? (e.type === "measurement" && e.data.weight_kg) : e.type === "weight")
    .map((e) => ({ t: new Date(e.ts).getTime(), v: U.mass.fromC(isBaby ? e.data.weight_kg : e.data.kg) }))
    .sort((a, b) => a.t - b.t);
...
function lineChart(pts, unit) {
  ...
  const sx = (t) => pad + (W - 2 * pad) * (maxX === minX ? 0.5 : (t - minX) / (maxX - minX));
  const sy = (v) => H - pad - (H - 2 * pad) * (maxY === minY ? 0.5 : (v - minY) / (maxY - minY));
...
function openAppts() {
  ...
  const now = Date.now();
  const up = appts.filter((e) => new Date(e.data.when).getTime() >= now).reverse();
  const past = appts.filter((e) => new Date(e.data.when).getTime() < now);
...
function applyTheme() {
  ...
  const meta = $('meta[name="theme-color"]'); if (meta) meta.content = t === "night" ? "#181310" : "#fbf3e7";
}
...
async function doLogin(form) {
  ...
  try {
    ...
    render(); syncAll();
  } catch (e) { if (err) err.textContent = String(e.message).includes("401") ? "That passphrase didn't work." : "Couldn't reach the server."; }
}
function logout() {
...
document.addEventListener("click", (ev) => {
  ...
  if (segBtn) {
    const segEl = segBtn.parentElement, seg = segEl.dataset.seg;
    segEl.querySelectorAll("button").forEach((b) => b.classList.toggle("on", b === segBtn));
    const hidden = segEl.parentElement.querySelector(`input[name="${seg}"]`) || document.querySelector(`#scrim input[name="${seg}"]`);
    if (hidden) hidden.value = segBtn.dataset.val;
    if (seg === "method") { const sc = $("#scrim"); sc.querySelector('[data-when="breast"]').style.display = segBtn.dataset.val === "breast" ? "" : "none"; sc.querySelector('[data-when="bottle"]').style.display = segBtn.dataset.val === "bottle" ? "" : "none"; }
  ...
  if (a.startsWith("nav:")) { state.view = a.slice(4); state.filter = "all"; render(); window.scrollTo(0, 0); }
  ...
  else if (a === "open-settings" || a === "open-settings") { state.view = "settings"; render(); }
...
document.addEventListener("submit", (ev) => {
  ...
  if (f.dataset.form === "login") doLogin(f);
    ...
    const d = formData(f);
    const patch = { baby_name: d.baby_name.trim(), due_date: d.due_date || null, birth_date: d.birth_date || null, units: d.units };
    const meVal = $("#me-input")?.value.trim(); if (meVal != null) { state.me = meVal; localStorage.setItem(LS.me, meVal); }
    saveSettings(patch).then(() => { render(); toast("Saved ✓"); });
  }
});

// close sheet by tapping the scrim backdrop
document.addEventListener("click", (ev) => { if (ev.target.id === "scrim") closeSheet(); });

function startSleep() { logEvent("sleep", {}, {}); closeSheet(); render(); toast("Sleep started 🌙"); }
function wakeBaby(id) {
  const e = state.events.find((x) => x.id == id); if (!e) return;
  patchEvent(id, { data: { ...e.data, end: new Date().toISOString() } }).then(() => toast("Good morning ☀️"));
}
function exportData() {
